...
namespace GSynchExt
{
	public class FundTransferApproval : PXBqlTable, IBqlTable, IAssignedMap
	{
		...
		[PXDefault]
		[PXDBInt]
		[PXSelector(typeof(Search<EPAssignmentMap.assignmentMapID, Where<EPAssignmentMap.entityType, Equal<AssignmentMapType.AssignmentMapTypeFTR>>>), SubstituteKey = typeof(EPAssignmentMap.name))]
		...
		[PXDBBool]
		[PXDefault(typeof(Search<GSynchExt.FundTransferRequestSetup.approvalMap>), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		public static class AssignmentMapType
		{
			...
			public class AssignmentMapTypeFTR : Constant<string>
			{
				public AssignmentMapTypeFTR() : base(typeof(GSynchExt.FundTransferRequest).FullName) { }
			}
