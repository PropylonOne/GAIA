...
namespace PX.Objects.IN
{
    public class INTransferEntryGSExt : PXGraphExtension<INTransferEntry>
    {
        ...
        public PXSelect<ProjectStock, 
            ...
            And<ProjectStock.siteID, Equal<Current<INRegister.siteID>>>>> ProjectStockItems;
        ...
        public virtual INRegister CreateTransfer(MaterialTransferRequest mTRequest, CopyDialogInfo info)
        {
            ...
            var hasLines = false;
            var req = PXSelect<MTRequestDetails, Where<MTRequestDetails.reqNbr,
            ...
            foreach (var MTRequestetails in req)
            {
                ...
                {
                    currentErrorMessage = PXMessages.LocalizeFormat("\nAll the requested qtys in {0} already trasnferred.\n", mTRequest.ReqNbr.Trim());
                }
                if (insertRecord)
                {
        ...
        #region CaseID 
        /// <summary>
        /// CASE ID - __________ - Need to Revise - Stopped the change because this will affect performance of the screen.
        /// </summary>
        /// <param name="e"></param>
        ...
        protected virtual void _(Events.RowSelected<INTran> e)
        {
            ...
            INItemCost itemCostRec = INItemCost.PK.Find(this.Base, row?.InventoryID, this.Base.Accessinfo.BaseCuryID);
            if (itemCostRec == null) return;

            decimal RoundedAvgCost = Math.Round((decimal)itemCostRec.AvgCost, 4);
            decimal RoundedQty = Math.Round((decimal)row.Qty, 4);
            rowExt.UsrISGAEstLineCost = (decimal)(RoundedAvgCost * RoundedQty);
