    ...
    public class ISGAFARequestEntry : PXGraph<ISGAFARequestEntry, ISGAFARequest>
    {
        ...
        public static EPEmployee GetCurrentEmployee(PXGraph graph)
        {
            EPEmployee currentEmployee = PXSelect<EPEmployee,
                                         ...
                                         .Select(graph);
            return currentEmployee ?? throw new PXException(GSynchExt.Messages.UserNotLinked);
        }
        ...
        // below is code change by Oshadhi
        protected virtual void _(Events.RowPersisted<ISGAFARequestLine> e)
        {
            ISGAFARequestLine row = e.Row;
            if (row == null) return;

            if (e.TranStatus == PXTranStatus.Completed && e.Operation == PXDBOperation.Insert)
            {
        ...
        protected virtual void _(Events.FieldUpdated<ISGAFARequestLine, ISGAFARequestLine.returnedQty> e)
        {
            ISGAFARequestLine row = e.Row;
            if (row != null)
            {
                row.BranchID = this.Accessinfo.BranchID;
            }
        ...
        Dictionary<(string, string), string> batchNumbersByKey = new Dictionary<(string, string), string>();

        protected void ISGAFARequestEvnt_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
        {
            var row = (ISGAFARequestEvnt)e.Row;
            if (row == null || string.IsNullOrWhiteSpace(row.ReqNbr) || string.IsNullOrWhiteSpace(row.ActionType))
                return;

            string prefix = row.ActionType.Substring(0, 2).ToUpper();

            var key = (row.ReqNbr, row.ActionType);

            if (!batchNumbersByKey.TryGetValue(key, out string batchNbr))
            {
                batchNbr = BatchNumberGenerator.GetNextBatchNumber(this, prefix, row.ActionType, row.ReqNbr);
                batchNumbersByKey[key] = batchNbr;
            }
        ...
        #region Batch Number Generater
        public static class BatchNumberGenerator
        {
            public static string GetNextBatchNumber(PXGraph graph, string prefix, string actionType, string reqNbr)
            {
                if (graph == null)
                    throw new ArgumentNullException(nameof(graph));

                if (string.IsNullOrWhiteSpace(prefix) || prefix.Length != 2 || !prefix.All(char.IsLetter))
                    throw new PXException(GSynchExt.Messages.PrefixMsg);

                if (string.IsNullOrWhiteSpace(reqNbr))
                    throw new PXException(GSynchExt.Messages.ReqNbrNotNull);

                prefix = prefix.ToUpper();
                string pattern = $"{prefix}-{reqNbr}-%";

                var existingBatches = PXSelect<
                    ISGAFARequestEvnt,
                    Where<
                        ISGAFARequestEvnt.reqNbr, Equal<Required<ISGAFARequestEvnt.reqNbr>>,
                        And<ISGAFARequestEvnt.actionType, Equal<Required<ISGAFARequestEvnt.actionType>>,
                        And<ISGAFARequestEvnt.batchNbr, Like<Required<ISGAFARequestEvnt.batchNbr>>>>>,
                    OrderBy<Desc<ISGAFARequestEvnt.batchNbr>>>
                    .SelectWindowed(graph, 0, 1, reqNbr, actionType, pattern)
                    .RowCast<ISGAFARequestEvnt>()
                    .ToList();
                ...
                {
                    var lastBatch = existingBatches.First();
                    string[] parts = lastBatch.BatchNbr?.Split('-');
                ...
                return $"{prefix}-{reqNbr}-{nextNumber.ToString("D5")}";
            }
        }
        #endregion
    }
