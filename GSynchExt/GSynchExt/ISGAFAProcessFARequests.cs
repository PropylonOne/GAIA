...

{
    ...
    public class ISGAFAProcessFARequests : PXGraph<ISGAFAProcessFARequests>
    {
        ...
        public override int ExecuteUpdate(string viewName, IDictionary keys, IDictionary values, params object[] parameters)
        {
            ...
            {
                if (viewName.ToLower() == "farequests" && ((ex is System.Data.SqlClient.SqlException && ((System.Data.SqlClient.SqlException)ex).Number == 208 /*invalid object name*/) || (ex is PXArgumentException && ((PXArgumentException)ex).ParamName == "table")))
                {
                    FARequests.Select();
                    try
                    {
    ...
    public class UpdateFARequestProcess : PXGraph<UpdateFARequestProcess>
    {
        public virtual void UpdateFARecords(ISGAFARequestItems rec)
        {
            using (new PXConnectionScope())
            {
                ...
                {
                    PXResultset<ISGAFARequestItems> item = PXSelect<ISGAFARequestItems>.Select(this);
